...
# Event-B Bank Account Project

**Miguel Smurawski, José Bustamante Morales, Pierre Thalamy**  
*Rigorous Software Development course, ETSIINF, Universidad Politécnica de Madrid*

## Model Architecture

- We decided to start with an initial abstract model containing the main variables, and requirements not related to any specific operations. We also start with an abstract event `operate` that represents an abstract operation on account ownership and balance.  
- Then, we made one refinement of the model for each operation to implement. Each concrete operation event is a refinement of the abstract `operate` event.
- Finally, the `bankAccount_M5_finalK` model implements the last requirement (**FUN10**) related to consecutive operations.
...
> The decrease of the balance of any account caused by two consecutive operations has to be less than or equal to `K` (for some constant `K`).
